...
We decided to start with a Random Forest model, to see if it would have acceptable performance.
...
```
The model has used 500 trees and decides to try 23 variables at each split.
...
The accuracy is 0.9917 so the predicted accuracy for the out-of-sample error is less than a 0.01%. As this is an excellent result, rather than trying additional algorithms, I will use Random Forests to predict on the test set.
